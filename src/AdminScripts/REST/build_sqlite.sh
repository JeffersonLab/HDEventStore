...
# pass run number in as command line argument
export RUN_NUMBER=$1
export rundir=`echo $RUN_NUMBER | perl -n -e 'printf "%06d", $_;'`

export INPUT_DIR="${EVENTSTORE_BASE_DIR}/files/${rundir}"
export OUT_DIR="${EVENTSTORE_BASE_DIR}/rest_index/${rundir}"
...
if [ -e ${INPUT_DIR}/rest_data_location.txt ]
then
    filesToInject=`cat ${INPUT_DIR}/rest_data_location.txt`
    firstFileToInject=`echo $filesToInject | awk '{print $1}'`
    log found rest_data_location.txt
else
...
if [ -e ${INPUT_DIR}/rest_eventstore_location.txt ]
then
    eventStoreOutputPath=`cat ${INPUT_DIR}/rest_eventstore_location.txt`
    log found rest_eventstore_location.txt
else
...
if [ -e ${INPUT_DIR}/rest_idxa_location.txt ]
then
    idxaListPath=`cat ${INPUT_DIR}/rest_idxa_location.txt`
    log found rest_idxa_location.txt
    echo $idxaListPath
else
...
echo files are: 
ls $filesToInject
# if the ls failed to find any files, it will return a non-zero exit code
# if there's no files, out job is done
if [[ $? -ne 0 ]]
then
    succeed
fi
...
do
    ...
    numberLinesFile=`wc -l $theIdxaFile | awk '{print $1;}'`
    if [ $numberLinesFile -le 1 ]
    then
	continue
    fi
...
heDbFile -output $eventStoreOutputPath -add $theIdxaFile -logFile $theIdxaLogFile
    mystatus=$?
    if [ $mystatus -eq 0 -a \
